...
BUDGET_ADAPTER_H = (ROOT / "tools/server/server-kv-budget-adapter.h").read_text(encoding="utf-8")
BUDGET_ADAPTER_CPP = (ROOT / "tools/server/server-kv-budget-adapter.cpp").read_text(encoding="utf-8")
PRESSURE_ACTION_CPP = (ROOT / "tools/server/server-kv-pressure-action.cpp").read_text(encoding="utf-8")
SERVER_CONTEXT = (ROOT / "tools/server/server-context.cpp").read_text(encoding="utf-8")
MOE_H = (ROOT / "src/llama-moe-buffer.h").read_text(encoding="utf-8")
MOE_CPP = (ROOT / "src/llama-moe-buffer.cpp").read_text(encoding="utf-8")
...
class MemoryGovernorObserveStaticTest(unittest.TestCase):
    ...
    def test_global_adapter_and_legacy_gate_contract(self):
        self.assertIn("server_kv_budget_adapter", SERVER_CONTEXT)
        self.assertIn("server_kv_budget_adapter::confirm_physical_credit", SERVER_CONTEXT)
        self.assertIn("reallocation_confirmed_kv_physical_credit_bytes", SERVER_CONTEXT)
        self.assertIn("reallocation_kv_physical_credit_earned_bytes", SERVER_CONTEXT)
        self.assertIn("LLAMA_MEMORY_GOVERNOR_LEGACY_KV_ACTIONS", SERVER_CONTEXT)
        self.assertIn("legacy_disabled", SERVER_CONTEXT)
        self.assertIn("budget_authority_mismatch", SERVER_CONTEXT + PRESSURE_ACTION_CPP)
        self.assertIn("budget_target_relaxed", SERVER_CONTEXT + PRESSURE_ACTION_CPP)
        self.assertIn("source_object_id", BUDGET_ADAPTER_H + PRESSURE_ACTION_CPP)
        self.assertIn("source_generation", BUDGET_ADAPTER_H + PRESSURE_ACTION_CPP)
        self.assertIn("kv_physical_mincore_delta", BUDGET_ADAPTER_CPP)
        self.assertIn("before.resident_bytes <= after.resident_bytes", BUDGET_ADAPTER_CPP)
        self.assertIn("!memory_governor_legacy_kv_actions_enabled", SERVER_CONTEXT)
    ...
    def test_memory_governor_kv_release_supersedes_old_unified_path(self):
        ...
        self.assertIn("server_kv_pressure_execute_governor", unified_path)
        self.assertIn("server_kv_budget_adapter", unified_path)
        self.assertIn("!memory_governor_legacy_kv_actions_enabled", SERVER_CONTEXT)
        self.assertIn("memory_governor_legacy_kv_actions_enabled && memory_governor_kv_release_enabled", SERVER_CONTEXT)
        self.assertIn("memory_governor_legacy_kv_actions_enabled && memory_governor_kv_offload_enabled", SERVER_CONTEXT)
