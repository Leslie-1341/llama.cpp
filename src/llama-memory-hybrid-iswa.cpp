...
void llama_memory_hybrid_iswa_context::clear_paged_swap_error() {
    if (ctx_attn) {
...
bool llama_memory_hybrid_iswa_context::has_paged_swap_error() const {
    return (ctx_attn && ctx_attn->has_paged_swap_error()) ||
           (ctx_recr && ctx_recr->has_paged_swap_error());
}

llama_paged_swap_error llama_memory_hybrid_iswa_context::get_paged_swap_error() const {
...
void llama_memory_hybrid_iswa_context::mark_paged_kv_compute_started() {
    if (ctx_attn) ctx_attn->mark_paged_kv_compute_started();
    if (ctx_recr) ctx_recr->mark_paged_kv_compute_started();
}

bool llama_memory_hybrid_iswa_context::finish_paged_kv_write(llama_paged_kv_write_action action) {
...
bool llama_memory_hybrid_iswa_context::needs_paged_kv_post_graph_sync() const {
    return (ctx_attn && ctx_attn->needs_paged_kv_post_graph_sync()) ||
           (ctx_recr && ctx_recr->needs_paged_kv_post_graph_sync());
}

bool llama_memory_hybrid_iswa_context::paged_kv_failure_handled() const {
    return paged_failure_handled;
}

bool llama_memory_hybrid_iswa_context::test_paged_kv_fail_graph_alloc() {
    if (ctx_attn && ctx_attn->test_paged_kv_fail_graph_alloc()) return true;
    return ctx_recr && ctx_recr->test_paged_kv_fail_graph_alloc();
}

bool llama_memory_hybrid_iswa_context::test_paged_kv_fail_after_compute() {
