...
struct common_params {
    ...
    bool vm_debug_log      = false; // dump VM region residency classification
    int32_t vm_block_size_mb       = 4;   // VM weight block size in MiB
    int32_t vm_pin_small_mb        = 2;   // VM small tensor pin threshold in MiB
    int32_t vm_pin_budget_mb       = 128; // VM total mlock budget in MiB
    int32_t vm_prefetch_budget_mb  = 512; // VM graph prefetch budget in MiB
    int32_t vm_window_steps        = 2;   // VM graph prefetch window in execution steps
    int32_t vm_window_layers       = 2;   // VM layer prefetch window
    int32_t vm_reclaim_budget_mb   = 256; // VM DONTNEED reclaim budget in MiB
    int32_t vm_keep_behind_steps   = 2;   // VM keep-behind steps for reclaim
    int32_t vm_keep_behind_layers  = 2;   // VM layer keep-behind window for reclaim
    int32_t vm_reclaim_policy      = 0;   // VM reclaim policy: 0 none, 1 dontneed, 2 free, 3 pageout
    int32_t vm_reclaim_distance    = 0;   // VM minimum layer distance before reclaim
    int32_t vm_keep_behind_groups  = 0;   // VM sliding-unmap groups kept behind current group
    int32_t vm_plan_cache_entries  = 8;   // VM max graph plan cache entries
    int32_t vm_pipeline_layers     = 2;   // VM subgraph pipeline prefetch layers
    int32_t vm_subgraph_group_layers = 1; // VM layers per submitted subgraph
    int32_t vm_subgraph_sync_depth = 1;   // VM submitted subgraphs per backend sync
    bool vm_dontneed      = false; // use MADV_DONTNEED for safe VM reclaim
    bool vm_layer_schedule = false; // use scheduler callbacks for layer-level VM scheduling
    bool vm_prefill_dontneed = false; // enable VM layer lifecycle/reclaim during prompt prefill
    bool vm_subgraph       = false; // use layer-wise subgraph submission
    bool vm_double_buffer  = false; // use double-buffered VM layer prefetch
    bool vm_subgraph_plan_path  = false; // use backend graph plan create/compute/free for each subgraph
    bool vm_subgraph_plan_cache = false; // cache and update backend graph plans for subgraphs
    bool vm_subgraph_sequence   = false; // execute cached CPU subgraph plans as one sequence
    bool vm_sliding_unmap       = false; // unmap completed layer groups and remap before reuse
    bool vm_hugepage       = false; // use transparent hugepage (THP) for lower TLB miss rate
    bool vm_hugepage_align = true;  // align mmap to 2 MiB boundaries for better THP success rate (default: auto-enabled with vm_hugepage)
