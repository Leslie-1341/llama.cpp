#!/usr/bin/env python3
"""Fail-closed verdict authority for G0-S1 and B0-B2 server baseline artifacts."""
...
def main_parse(root: pathlib.Path) -> tuple[str, list[str]]:
    try:
        ...
        return "FAIL", [str(exc)]
    if isinstance(manifest, dict) and manifest.get("protocol") == BASELINE_PROTOCOL:
...
def main() -> None:
    ...
    for detail in details:
        print(f"- {detail}")
    raise SystemExit(0 if status in {"PASS", "DRY_RUN"} else 4 if status == "DIAGNOSTIC" else 3 if status == "UNSUPPORTED" else 1)
